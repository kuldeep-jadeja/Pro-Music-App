...
export async function enqueue(fnOrJob) {
    ...
    // ── Redis queue path ─────────────────────────────────────────────────
    const enqueued = await enqueueYouTubeMatch(fnOrJob);
    if (!enqueued) {
        throw new Error('YouTube match queue unavailable');
    }
}
...
export async function batchMatchTracks(tracksToMatch, playlistId, delayMs = 1000) {
    ...
    for (const track of tracksToMatch) {
        try {
            ...
            // Queue failure is fatal for this playlist run: pause immediately so
            // status is terminal (not stuck in "matching") and can be resumed.
            console.error(
            ...
            await Playlist.updateOne(
                { _id: playlistId },
                {
                    $set: {
                        status: 'paused',
                        errorMessage: `YouTube matching paused: ${err.message}`,
                        pausedAt: new Date(),
