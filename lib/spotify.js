...
async function fetchPlaylistTracksPage(playlistId, offset, limit, accessToken) {
    const fields = encodeURIComponent(
        'items(track(id,name,artists(name),album(name,images),duration_ms,uri))'
    );
    const url =
...
export async function getPublicPlaylistData(playlistId) {
    ...
    if (data.tracks?.items && Array.isArray(data.tracks.items)) {
        ...
        // Format B always includes data.tracks.total; if it exceeds what we got,
        // fetch the remaining pages via the anonymous Web API token.
        const total = data.tracks.total;
        const initialCount = data.tracks.items.length;
        if (typeof total === 'number' && total > initialCount) {
            console.log(
                `[Spotify] Playlist has ${total} tracks, initial page has ${initialCount} — paginating...`
            );
            try {
                const token = await getSpotifyAccessToken();
                const PAGE_LIMIT = 100;
                for (let offset = initialCount; offset < total; offset += PAGE_LIMIT) {
                ...
                console.log(
                    `[Spotify] Pagination complete — ${tracks.length} tracks fetched.`
                );
            } catch (paginationErr) {
                // Pagination failed — keep whatever the initial page returned.
                // Partial import is better than a hard failure.
                console.warn(
                    `[Spotify] Pagination failed after ${tracks.length} tracks:`,
