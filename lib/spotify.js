...
// ── Tier 2: Spotify OG Scrape ─────────────────────────────────────

/**
 * Fetch album name + art by scraping the Open Graph meta tags from
 * open.spotify.com/track/{spotifyId}.  This works for 100 % of tracks
 * because we already have the spotifyId for every imported song.
 *
...
async function fetchFromMusicBrainz(track) {
    ...
    const query = encodeURIComponent(
        `recording:"${track.name}" AND artist:"${artist}"`
    );
    const mbUrl =
...
export async function runBackgroundItunesEnrichment(tracks) {
    ...
    try {
        ...
        const bulkOps = needsEnrichment
            .filter((t) => t.album || t.albumImage)
            .map((t) => ({
                updateOne: {
                    filter: { spotifyId: t.spotifyId },
                    update: { $set: { album: t.album, albumImage: t.albumImage } },
                },
            }));
 ...
 *
 * Tier execution:
 *   1. iTunes         — up to 5 concurrent, 300 ms delay between batches
 *   2. Spotify OG     — up to 3 concurrent, 500 ms delay between batches (for remaining)
 *   3. MusicBrainz + CoverArtArchive — serialised at ~1 req/s (for remaining)
 *
...
async function enrichTracksWithMetadata(tracks) {
    ...
    // Tracks that iTunes couldn't fully resolve
    const afterItunes = tracks.filter(
        (t) => !t.albumImage || !t.album || t.album === 'Unknown Album'
    );
    if (afterItunes.length === 0) {
        console.log('[Enrichment] iTunes resolved all tracks — Spotify OG not needed.');
        return;
    }
    console.log(`[Enrichment] iTunes missed ${afterItunes.length} track(s) — trying Spotify OG...`);

    // ── Tier 2: Spotify OG scrape ───────────────────────────────────
    // Lower concurrency (3) + larger delay (500 ms) to be polite to Spotify's servers.
    // This tier has ~100 % coverage because every track has a spotifyId.
    const OG_CONCURRENCY = 3;
    const OG_BATCH_DELAY = 500;

    for (let i = 0; i < afterItunes.length; i += OG_CONCURRENCY) {
        await Promise.all(
            afterItunes.slice(i, i + OG_CONCURRENCY).map(fetchFromSpotifyOG)
        );
        if (i + OG_CONCURRENCY < afterItunes.length) {
            await sleep(OG_BATCH_DELAY);
        }
    }

    const afterOG = afterItunes.filter(
        (t) => !t.albumImage || !t.album || t.album === 'Unknown Album'
    );
    if (afterOG.length === 0) {
        console.log('[Enrichment] Spotify OG resolved all remaining tracks.');
        return;
    }
    console.log(`[Enrichment] Spotify OG missed ${afterOG.length} track(s) — trying MusicBrainz...`);

    // ── Tier 3: MusicBrainz + CoverArtArchive (serialised, 1 req/s) ─
    const MB_DELAY = 1100; // stay safely under MusicBrainz's 1 req/s limit

    for (let i = 0; i < afterOG.length; i++) {
        await fetchFromMusicBrainz(afterOG[i]);
        if (i < afterOG.length - 1) {
            await sleep(MB_DELAY);
        }
    }

    const stillMissing = afterOG.filter(
        (t) => !t.albumImage || !t.album || t.album === 'Unknown Album'
    );
    if (stillMissing.length > 0) {
        console.warn(
            `[Enrichment] ${stillMissing.length} track(s) unresolved after all 3 tiers:`,
