...
/**
 * Fetch an anonymous Spotify access token from the public web-player endpoint.
 * Spotify issues short-lived tokens to any browser — no developer account needed.
 */
async function getSpotifyAccessToken() {
    ...
    if (!res.ok) throw new Error(`[Spotify] Access token request failed: ${res.status}`);
    const json = await res.json();
    if (!json.accessToken) throw new Error('[Spotify] No accessToken in token response');
