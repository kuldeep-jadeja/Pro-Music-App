...
/**
 * Extract an anonymous Spotify access token by fetching the playlist embed page.
 * The token lives in __NEXT_DATA__ at state.settings.session.accessToken.
 * Spotify's /get_access_token endpoint is blocked in server environments — this
 * approach works because the embed page is always publicly accessible.
 *
 * @param {string} playlistId  Spotify playlist ID (used to fetch the embed page)
 */
async function getSpotifyAccessToken(playlistId) {
    ...
    if (!res.ok) throw new Error(`[Spotify] Embed page request failed: ${res.status}`);
    const html = await res.text();
    const match = html.match(/<script id="__NEXT_DATA__"[^>]*>([^<]+)<\/script>/);
    if (!match) throw new Error('[Spotify] __NEXT_DATA__ not found in embed page');
    const nd = JSON.parse(match[1]);
    const token = nd?.props?.pageProps?.state?.settings?.session?.accessToken;
    if (!token) throw new Error('[Spotify] accessToken not found in embed page __NEXT_DATA__');
    return token;
}

/**
 * Fetch one page of playlist tracks from the Spotify Web API.
 * Requires the anonymous token from getSpotifyAccessToken().
 */
async function fetchPlaylistTracksPage(playlistId, offset, limit, accessToken) {
    ...
    if (!res.ok) throw new Error(`[Spotify] Tracks page request failed: ${res.status}`);
    return res.json();
}

/**
 * Fetch remaining pages of playlist tracks from the Spotify Web API and
 * append them to `existingTracks`. Returns the same (mutated) array.
 *
 * Pass `knownTotal = null` when the embed format has no total count (Format A).
 * In that case the real total is read from the first API response — if it equals
 * `startOffset`, no pages are fetched and we return immediately.
 *
 * Never throws: pagination errors are logged and the partial list is returned.
 */
async function paginatePlaylistTracks(existingTracks, playlistId, startOffset, knownTotal) {
    try {
        const token = await getSpotifyAccessToken(playlistId);
        const PAGE_LIMIT = 100;
        let offset = startOffset;
        let total = knownTotal ?? Infinity; // updated from first API response when null

        if (knownTotal != null) {
            console.log(
                `[Spotify] Playlist has ${knownTotal} tracks, embed returned ${startOffset} — paginating...`
            );
        } else {
            console.log(
                `[Spotify] Embed returned ${startOffset} tracks with no total — checking for more via API...`
            );
        }

        while (offset < total) {
            const page = await fetchPlaylistTracksPage(playlistId, offset, PAGE_LIMIT, token);

            // For Format A (knownTotal = null) read the real total from the first response
            if (typeof page.total === 'number' && total === Infinity) {
                total = page.total;
                if (total <= startOffset) {
                    console.log(
                        `[Spotify] API confirmed ${total} total tracks — no additional pages needed.`
                    );
                    break;
                }
            }
            ...
            existingTracks.push(...pageTracks);
            offset += pageTracks.length;
            if (pageTracks.length < PAGE_LIMIT) break; // last page
        }

        console.log(`[Spotify] Pagination complete — ${existingTracks.length} total tracks.`);
    } catch (paginationErr) {
        console.warn(
            `[Spotify] Pagination failed after ${existingTracks.length} tracks:`,
            paginationErr.message
        );
    }
...
export async function getPublicPlaylistData(playlistId) {
    ...
    if (data.tracks?.items && Array.isArray(data.tracks.items)) {
        ...
        // Paginate when Spotify only returned a partial first page.
        // Format B includes data.tracks.total; if it exceeds what we got, fetch the rest.
        const total = data.tracks.total;
        if (typeof total === 'number' && total > tracks.length) {
            await paginatePlaylistTracks(tracks, playlistId, tracks.length, total);
        }
    }
    ...
    // Format A also has NO total count. If the embed returned exactly 100 tracks
    // the playlist may be longer — call the Spotify API to find out and fetch the rest.
    else if (data.trackList && Array.isArray(data.trackList)) {
        tracks = data.trackList.map(parseEmbedTrack).filter(Boolean);

        if (tracks.length === 100) {
            await paginatePlaylistTracks(tracks, playlistId, 100, null);
        }
    }
