...
/**
 * Background iTunes enrichment — call this AFTER saving tracks to MongoDB
 * and responding to the client.  Enriches tracks missing album name / art
 * via the free iTunes Search API, then persists the results to MongoDB.
 *
...
export async function runBackgroundItunesEnrichment(tracks) {
    ...
    console.log(`[iTunes] Background enrichment starting for ${needsEnrichment.length} tracks...`);

    try {
        await enrichTracksWithItunes(needsEnrichment);

        // Persist enriched fields back to MongoDB using spotifyId as the key.
        // We only update tracks that actually received new data.
        ...
        if (bulkOps.length > 0) {
            ...
            console.log(`[iTunes] Persisted enrichment data for ${bulkOps.length} tracks.`);
        }
    } catch (err) {
        // Enrichment failures must not surface to the user — import already succeeded.
        console.error('[iTunes] Background enrichment error:', err.message);
    }
}
